...
void processpool<T, P>::run_parent(const algorithmservice::business::Config &config)
{
    ...
    auto func = [this](const std::string &path, bool is_dir) -> bool {
        log_print(kTrace, "traversing task directory, find: %s, is dir: %d", path.c_str(), is_dir);
        if (!is_dir && ustd::get_last_path(path)[0] != '.')
        {
            std::shared_ptr<protocol::algorithmservice::mosquitto_message> msg = protocol::algorithmservice::BoostBinaryDerializeFromFile<protocol::algorithmservice::mosquitto_message>(path);
            int i = select_process(SelectMode::RandomFree);
            if (i < 0)
            {
                log_print(kTrace, "error loading task");
                return false;
            }
            // 下面的代码重复了，可以抽出来
            // 设置进程属性：
            ...
            printf("send unfinished task to child %d with fd %d\n", i, m_sub_process[i].m_pipefd[0]);
        }

        return true;
    };
    ustd::traverse_directory(m_config.save_config.msg, func);
    ...
    while (!m_stop)
    {
        ...
        for (int i = 0; i < number; i++)
        {
            ...
            if (sockfd == sig_mqfd[0])
            {
                ...
                {
                    std::string json_ret;
                    generate_err_json(msg->m_payload, "overflow", "no available service", json_ret);
